...
public class RoundManager : MonoBehaviour
{
    ...
    public const float roundDuration = 10f; // Duração de cada rodada
    ...
    public const float resetDelay = 5f; // Tempo entre o final da rodada e o início da próxima
    ...
    void EndRound()
    {
        ...
        {
            Debug.LogError(
                "currentQuestion é null. Não foi possível determinar a resposta correta."
            );
        ...
        foreach (var entry in playerStatus)
        {
            ...
            {
                Debug.Log(
                    $"Rodada {currentRound}: Jogador {player.name} ganhou um ponto por permanecer no hexágono!"
                );
                if (hudManager != null)
                {
            ...
            {
                Debug.Log(
                    $"Rodada {currentRound}: Jogador {player.name} caiu e não ganhou pontos!"
                );
            }
        }
    ...
    void CheckForWinner()
    {
        ...
        if (winners.Count > 0)
        {
            GameData.lastQuizSkyWinnersIndexes = winners;
            EndMinigame(winners);
        }
    }

    void EndMinigame(List<int> winners)
    {
